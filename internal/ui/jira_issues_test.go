...
func (f *fakeIssuesLoader) ListMyIssuesInProject(key string) ([]jira.Issue, error) {
	f.lastKey = key
...
func TestJiraIssues_Init_CallsWithProjectKey(t *testing.T) {
	...
	m := NewJiraIssuesModel(loader, jira.Project{Key: "GAP", Name: "Squad GAP"})
	cmd := m.Init()
...
func TestJiraIssues_GroupsByStatus(t *testing.T) {
	m := NewJiraIssuesModel(&fakeIssuesLoader{}, jira.Project{Key: "GAP", Name: "Squad GAP"})
	m.SetSize(120, 40)
	m, _ = m.Update(jiraIssuesLoadedMsg{issues: []jira.Issue{
...
func TestJiraIssues_LoadError(t *testing.T) {
	m := NewJiraIssuesModel(&fakeIssuesLoader{}, jira.Project{Key: "GAP"})
...
func TestJiraIssues_EmptyState(t *testing.T) {
	m := NewJiraIssuesModel(&fakeIssuesLoader{}, jira.Project{Key: "X", Name: "X"})
...
func TestJiraIssues_EscGoesBack(t *testing.T) {
	m := NewJiraIssuesModel(&fakeIssuesLoader{}, jira.Project{Key: "X"})
...
func TestFormatIssueLine_TruncatesLongSummary(t *testing.T) {
	iss := jira.Issue{Key: "X-1", Type: "Task", Summary: strings.Repeat("a", 200)}
